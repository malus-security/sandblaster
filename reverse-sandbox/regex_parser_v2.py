...
def parse_end(node_type, node_arg, node_transition, node_idx):
    return {"pos": node_idx, "nextpos": node_transition, "type": "end", "value": 0}
...
def parse_parantheses_open(node_type, node_arg, node_transition, node_idx):
    return parse_jump_backward(node_type, node_arg, node_transition, node_idx)
    """
    return {
        "pos": node_idx,
        "nextpos": node_transition,
...
def node_parse(re, i, regex_list, node_idx):
    node_type = struct.unpack("<B", "".join([chr(x) for x in re[i : i + 1]]))[0]
    node_transition = struct.unpack("<H", "".join([chr(x) for x in re[i + 1 : i + 3]]))[
        0
    ]
    pad = struct.unpack("<B", "".join([chr(x) for x in re[i + 3 : i + 4]]))[0]
    node_arg = struct.unpack("<I", "".join([chr(x) for x in re[i + 4 : i + 8]]))[0]
    i += 8

    logger.debug(
        "node idx:{:#06x} type: {:#02x} arg: {:#010x}"
        " transition: {:#06x}".format(node_idx, node_type, node_arg, node_transition)
    )
...
def classes_parse(re, i, cclass_count):
    ...
    classes_magic, classes_size = struct.unpack(
        "<II", "".join([chr(x) for x in re[i : i + 8]])
    )
    i += 0x8
    logger.debug(
        "classes magic = {:#x} size = {:#x}".format(classes_magic, classes_size)
    )
    assert len(re) - i == classes_size
    starts = struct.unpack(
        "<{}I".format(cclass_count),
        "".join([chr(x) for x in re[i : i + 4 * cclass_count]]),
    )
    i += 0x4 * cclass_count

    lens = struct.unpack(
        "<{}B".format(cclass_count), "".join([chr(x) for x in re[i : i + cclass_count]])
    )
    i += cclass_count
...
class RegexParser(object):
    ...
    def parse(re, i, regex_list):
        magic = struct.unpack("<I", "".join([chr(x) for x in re[i : i + 0x4]]))[0]
        logger.debug("magic = {:#x}".format(magic))

        node_count = struct.unpack(
            "<I", "".join([chr(x) for x in re[i + 0x4 : i + 0x8]])
        )[0]
        logger.debug("node count = {:#x}".format(node_count))

        start_node = struct.unpack(
            "<I", "".join([chr(x) for x in re[i + 0x8 : i + 0xC]])
        )[0]
        logger.debug("start node = {:#x}".format(start_node))

        end_node = struct.unpack(
            "<I", "".join([chr(x) for x in re[i + 0xC : i + 0x10]])
        )[0]
        logger.debug("end node = {:#x}".format(end_node))

        cclass_count = struct.unpack(
            "<I", "".join([chr(x) for x in re[i + 0x10 : i + 0x14]])
        )[0]
