...
def get_filter_arg_string_by_offset_with_type(f, offset):
    ...
    if type == 0x00 or type == 0x04 or type == 0x02:
        ...
        logger.warn(
            "Unknown type for string type: {} at offset {}".format(type, offset * 8)
        )
    actual_string = f.read(len)
    if (
        actual_string == "/private/var/tmp/launchd/sock"
...
def convert_filter_callback(
    f,
    ios_major_version_arg,
    keep_builtin_filters_arg,
    global_vars_arg,
    re_list,
    filter_id,
    filter_arg,
    ...
    if result == None and not (
        ...
        or (
            filter["name"] in ["extension", "mach-extension"] and ios_major_version <= 5
        )
    ):
        logger.warn(
