...
def node_parse(re, i, regex_list, node_idx):
    node_type = struct.unpack(">I", "".join([chr(x) for x in re[i : i + 4]]))[0]
    node_transition = struct.unpack(">I", "".join([chr(x) for x in re[i + 4 : i + 8]]))[
        0
    ]
    node_arg = struct.unpack(">I", "".join([chr(x) for x in re[i + 8 : i + 12]]))[0]
    i += 12

    logger.debug(
        "node idx:{:#010x} type: {:#02x} arg: {:#010x}"
        " transition: {:#010x}".format(node_idx, node_type, node_arg, node_transition)
    )
...
def class_parse(re, i, classes, class_idx):
    ...
    class_size = struct.unpack(">I", "".join([chr(x) for x in re[i : i + 4]]))[0]
    i += 0x4
    content = struct.unpack(
        ">{}I".format(class_size), "".join([chr(x) for x in re[i : i + 4 * class_size]])
    )
    i += 0x4 * class_size
    ...
    logger.debug(
        "class idx = {:#x} size = {:#x} content=[{}]".format(class_idx, class_size, cls)
    )
    classes.append(cls)
    return i
...
class RegexParser(object):
    ...
    def parse(re, i, regex_list):
        node_count = struct.unpack(">I", "".join([chr(x) for x in re[i : i + 0x4]]))[0]
