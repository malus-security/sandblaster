...
def parse_character(re, i, regex_list):
    ...
    if value == ".":
        value = "[.]"
    regex_list.append(
        {"pos": i - 6, "nextpos": i + 2 - 6, "type": "character", "value": value}
    )
    return i + 1
...
def parse_beginning_of_line(i, regex_list):
    regex_list.append(
        {"pos": i - 6, "nextpos": i + 1 - 6, "type": "character", "value": "^"}
    )
...
def parse_jump_forward(re, i, regex_list):
    ...
    regex_list.append(
        {"pos": i - 6, "nextpos": i + 3 - 6, "type": "jump_forward", "value": jump_to}
    )
    return i + 2
...
def parse_jump_backward(re, i, regex_list):
    jump_to = re[i + 1] + (re[i + 2] << 8)
    regex_list.append(
        {"pos": i - 6, "nextpos": i + 3 - 6, "type": "jump_backward", "value": jump_to}
    )
    logger.debug(
...
def parse_end(re, i, regex_list):
    regex_list.append({"pos": i - 6, "nextpos": i + 2 - 6, "type": "end", "value": 0})
    return i + 1
